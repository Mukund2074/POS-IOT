...
const FimageUpload = ({ formik, setter, id, field, handleFileRemove }) => {
    return (
        <div className="image-upload-container" style={{ marginTop: 0, mb: 0, marginLeft: '20px' }}>
            <input
                type="file"
                accept="image/*"
                onChange={(e) => {
            ...
            <label htmlFor={id} className="image-upload-label">
                <div className="image-upload-icon" style={{ padding: 0, marginBottom: 0, height: 80, width: '100%' }}>
                    {formik.values[id] ? (
                        <img
                            src={
                ...
                <Typography variant="caption" sx={{ color: '#6F6F6F' }}>
                    {formik.values[id] ? formik.values[id]?.name : t('Setting.UploadEmployeeImage')}
                </Typography>
            </label>
