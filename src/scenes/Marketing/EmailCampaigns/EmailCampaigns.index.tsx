...
import { resetCampaignForm } from '@/redux/slices/Marketing/campaigns';
...
export default function EmailCampaigns() {
    ...
    const getApiSortField = (columnId: string): GetApiCampaignsSortBy | null => {
        ...
        // Check if columnId is directly a valid GetApiCampaignsSortBy value
    ...
    // Fetch EMAIL campaigns using the hook
    const { campaigns, isLoading, pagination } = useGetCampaigns({
        ...
        campaignType: GetApiCampaignsCampaignType.EMAIL,
    ...
    return (
        <React.Fragment>
            <div className="mb-2 md:mb-6">
                <h1 className="text-lg font-semibold text-text-primary m-0 md:mb-1">
                    {EMAIL_CAMPAIGN_CONFIG.translationKeys.title}
                </h1>
                <p className="text-sm text-text-secondary m-0">{EMAIL_CAMPAIGN_CONFIG.translationKeys.description}</p>
            </div>
            ...
            <RadixCard className="mt-4 md:mt-6 !p-0">
                <div className="flex items-center justify-between px-1 pt-0 pb-2 border-b border-border-default">
                    <h2 className="text-lg font-semibold text-text-primary m-0">
                        {EMAIL_CAMPAIGN_CONFIG.translationKeys.createdCampaigns}
                    </h2>
                    <RadixButton onClick={handleCreateCampaign} className="min-w-fit md:min-w-0">
                        {isDesktop ? EMAIL_CAMPAIGN_CONFIG.translationKeys.createCampaign : t('GiftCard.Create')}
                    </RadixButton>
                </div>
                <CampaignsList
                    campaigns={campaigns}
